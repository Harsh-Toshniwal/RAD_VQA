...
def train(args, model,question_model, train_loader, eval_loader,s_opt=None, s_epoch=0):
    ...
    for epoch in range(s_epoch, args.epochs):
        ...
        # Checkpoint at every 50 epochs
        if (epoch + 1) % 50 == 0:
            checkpoint_path = os.path.join(ckpt_path, f'checkpoint_epoch_{epoch}.pth')
            utils.save_model(checkpoint_path, model, epoch, optim)
            logger.info(f'[Checkpoint] Saved model at epoch {epoch}')
        ...
        if eval_loader is not None:
            ...
            logger.info(f'[Early Stopping] Epochs without improvement: {epochs_no_improve}/{early_stopping_patience}')
            
            # Early stopping
            if epochs_no_improve >= early_stopping_patience:
                logger.info(f'[Early Stopping] No improvement for {early_stopping_patience} epochs. Stopping training.')
                logger.info(f'[Final Result] Best accuracy: {best_eval_score:.6f}% at epoch {best_epoch}')
                break
